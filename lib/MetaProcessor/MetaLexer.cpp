...
Lexer::Lexer(llvm::StringRef line, bool skipWhite)
  : bufferStart(line.data()), curPos(line.data()) {
  if (skipWhite)
    SkipWhitespace();
}

void Lexer::reset(llvm::StringRef line) {
...
void Lexer::LexAnyString(Token& Tok) {
  Tok.startToken(curPos);
...
void Lexer::LexPunctuator(const char* C, Token& Tok) {
  Tok.startToken(C);
  Tok.setLength(1);
  switch (*C) {
...
void Lexer::LexPunctuatorAndAdvance(const char*& curPos, Token& Tok) {
  Tok.startToken(curPos);
  while (true) {
    ...
    Lexer::LexPunctuator(curPos++, Tok);
    if (Tok.isNot(tok::unknown))
      return;
  }
}

void Lexer::LexQuotedStringAndAdvance(const char*& curPos, Token& Tok) {
...
void Lexer::LexConstant(char C, Token& Tok) {
  while (C >= '0' && C <= '9')
    C = *curPos++;
...
void Lexer::LexIdentifier(char C, Token& Tok) {
...
void Lexer::LexEndOfFile(char C, Token& Tok) {
  if (C == '\0') {
    Tok.setKind(tok::eof);
    Tok.setLength(1);
  }
...
void Lexer::LexWhitespace(char C, Token& Tok) {
  SkipWhitespace();
