...
class meta::Processor::RedirectOutput {
  struct Redirect {
    int FD;
  ...
  int restore(int FD, FILE *F, RedirectionScope Flag, int bakFD) {
...
public:
  ...
  void redirect(llvm::StringRef filePath, bool append, RedirectionScope scope) {
    if (filePath.empty()) {
      ...
      for (auto it = m_Stack.rbegin(), e = m_Stack.rend(); it != e; ++it) {
        ...
        if (Match) {
          ...
          R->Scope = RedirectionScope(R->Scope & ~Match);
          ...
          lScope = RedirectionScope(lScope & ~Match);
      ...
      if (push(new Redirect(filePath.str(), append, scope)) != kInvalidFD) {
        ...
        if (scope & Processor::kSTDOUT && m_Backup[0] == kInvalidFD)
          m_Backup[0] = ::dup(STDOUT_FILENO);
        if (scope & Processor::kSTDERR && m_Backup[1] == kInvalidFD)
          m_Backup[1] = ::dup(STDERR_FILENO);
      } else
        return; // Failure
    }

    if (scope & Processor::kSTDOUT)
      m_CurStdOut =
          restore(STDOUT_FILENO, stdout, Processor::kSTDOUT, m_Backup[0]);
    if (scope & Processor::kSTDERR)
      restore(STDERR_FILENO, stderr, Processor::kSTDERR, m_Backup[1]);
  }
...
int Processor::process(const char* input_text,
                       Interpreter::CompilationResult& compRes, Value* result) {
  if (result)
    *result = Value();
...
void Processor::setStdStream(llvm::StringRef file, RedirectionScope scope,
                             bool append) {
