...
namespace {
  ...
  static bool falseWithError(const char* err) {
    llvm::errs() << "ValueExtractionSynthesizer error: " << err << ".\n";
    return false;
  }
  ...
  bool ValueExtractionSynthesizer::FindAndCacheRuntimeDecls() {
    ...
    if (m_Sema->getLangOpts().CPlusPlus) {
      if (!(NSD = utils::Lookup::Namespace(m_Sema, "cling")))
        return falseWithError("cling namespace not defined");
      if (!(NSD = utils::Lookup::Namespace(m_Sema, "runtime", NSD)))
        return falseWithError("cling::runtime namespace not defined");
      if (!(clingVD = cast<VarDecl>(utils::Lookup::Named(m_Sema, "gCling",
                                                            NSD))))
        return falseWithError("cling::runtime::gCling not defined");
      if (!NSD)
        return falseWithError("cling::runtime namespace not defined");

      if (!(NSD = utils::Lookup::Namespace(m_Sema, "internal", NSD)))
        return falseWithError("cling::runtime::internal namespace not defined");
    }
    ...
    if (R.empty())
      return falseWithError("Cannot find cling::runtime::internal::"
                           "::setValueNoAlloc");
    ...
    if (!(m_UnresolvedWithAlloc
            = m_Sema->BuildDeclarationNameExpr(CSS, R, /*ADL*/ false).get()))
      return falseWithError("Cluld not build cling::runtime::internal::setValueWithAlloc");
    ...
    if (!m_isChildInterpreter && R.empty())
      return falseWithError("Cannot find cling::runtime::internal::copyArray");

    if (!(m_UnresolvedCopyArray
           = m_Sema->BuildDeclarationNameExpr(CSS, R, /*ADL*/ false).get()))
      return falseWithError("Could not build cling::runtime::internal::copyArray");
