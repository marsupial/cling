...
static std::string callCPrintValue(const Value& V, const void* Val,
  ...
  const clang::FunctionDecl* FD =
      LH.findFunction(nullptr, Strm.str(), LookupHelper::WithDiagnostics);
  if (!FD)
    return printAddress(Val);
  ...
  Value CallReturn;
  cling::utils::SupressDiagnostics Supress(
      Interp->get<clang::DiagnosticsEngine, clang::LangOptions>(),
      cling::utils::SupressDiagnostics::kDiagnostics);
...
static std::string printUnpackedClingValue(const Value &V) {
  ...
  return callPrintValue(V, V.getPtr(), &Ty);
}
...
static std::string cling_PrintValue(const Value& Val) {
  ...
  cling::smallstream Strm;
  Strm << valuePrinterInternal::kInvalid << ' ' << printAddress(&Val, '@');
  return Strm.str();
}
