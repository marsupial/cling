...
namespace cling {
  IncrementalParser::IncrementalParser(Interpreter* interp,
                                       ...
                                       const char* llvmdir):
    m_Interpreter(interp), m_Consumer(nullptr), m_ModuleNo(0),
    m_PragmaHandler(nullptr) {

    auto ci = CIFactory::createCI("", argc, argv, llvmdir);
    m_CI.reset(ci.first);
    if (!m_CI) {
      llvm::errs() << "Compiler instance could not be created.\n";
      return;
    } else if (!ci.second) {
      interp->getOptions().HadOutput = true;
      return;
    }
  ...
  IncrementalParser::Initialize(llvm::SmallVectorImpl<ParseResultTransaction>&
    ...
    if (!PCHFileName.empty()) {
      ...
      // TODO: Better diagnosis of whether the error is safe to ignore.
